...
>
    ...
    >
        ...
        <div class="p-6">
            {{-- Newsletter Form --}}
            <form @submit.prevent="submitNewsletter()" class="mb-6">
            ...
            <div class="border-t border-gray-100 pt-4">
                <p class="text-gray-500 text-sm text-center mb-4">Ou rejoignez-nous sur</p>
                <div class="grid grid-cols-2 gap-3">
                    @foreach($channels as $channel)
                    <a
                        href="{{ $channel->url }}"
                        target="_blank"
                        @click="trackChannelClick('{{ $channel->type }}')"
                        class="flex items-center justify-center gap-2 px-4 py-3 rounded-lg border-2 transition hover:shadow-md
                            {{ $channel->type === 'whatsapp' ? 'border-green-500 bg-green-50 text-green-700 hover:bg-green-100' : '' }}
                            {{ $channel->type === 'telegram' ? 'border-blue-500 bg-blue-50 text-blue-700 hover:bg-blue-100' : '' }}
                            {{ $channel->type === 'facebook' ? 'border-blue-600 bg-blue-50 text-blue-800 hover:bg-blue-100' : '' }}
                            {{ $channel->type === 'instagram' ? 'border-pink-500 bg-pink-50 text-pink-700 hover:bg-pink-100' : '' }}
                        "
                    >
...
function leadCapture(frequency, delay) {
    return {
        ...
        neverShow() {
            localStorage.setItem('lead_capture_dismissed', 'true');
        },
        ...
        trackChannelClick(type) {
            fetch('/api/marketing/track-channel', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content
                },
                body: JSON.stringify({ type, source: 'popup' })
            }).catch(() => {});
        }
    };
}
